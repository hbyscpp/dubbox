...
public class DecodeableRpcInvocation extends RpcInvocation implements Codec, Decodeable {
  ...
  public Object decode(Channel channel, InputStream input) throws IOException {
    ObjectInput in =
        CodecSupport.getSerialization(channel.getUrl(), serializationType).deserialize(
        ...
        if (argNum >= 0) {
          if (argNum == 0) {
            ...
            for (int i = 0; i < args.length; i++) {
              try {
                if (in instanceof KryoObjectInput) {
                  pts[i] = ((KryoObjectInput) in).readClass();
                  args[i] = ((KryoObjectInput) in).readClassObject(pts[i]);
                } else {
