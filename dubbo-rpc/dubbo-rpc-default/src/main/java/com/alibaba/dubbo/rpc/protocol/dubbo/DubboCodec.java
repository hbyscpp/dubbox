...
public class DubboCodec extends ExchangeCodec implements Codec2 {
  ...
  public static final String DUBBO_VERSION = Version.getVersion(DubboCodec.class,
      Version.getVersion());
        ...
        try {
          ...
          if (res.isHeartbeat()) {
            ...
            if (channel.getUrl().getParameter(Constants.DECODE_IN_IO_THREAD_KEY,
              ...
              result =
                  new DecodeableRpcResult(channel, res, is, (Invocation) getRequestData(id), proto);
              result.decode();
            } else {
              result =
                  new DecodeableRpcResult(channel, res, new UnsafeByteArrayInputStream(
      ...
      try {
        ...
        if (req.isHeartbeat()) {
          ...
          if (channel.getUrl().getParameter(Constants.DECODE_IN_IO_THREAD_KEY,
            ...
            inv =
                new DecodeableRpcInvocation(channel, req, new UnsafeByteArrayInputStream(
  ...
  private boolean containComplexArguments(RpcInvocation invocation) {
    for (int i = 0; i < invocation.getParameterTypes().length; i++) {
      ...
      if (invocation.getParameterTypes()[i] != invocation.getArguments()[i].getClass()) {
        // modified by chenjunpeng
        // exclude base type
        Class<?> baseType = invocation.getParameterTypes()[i];
        if (baseType == int.class || baseType == byte.class || baseType == short.class
            || baseType == long.class || baseType == float.class || baseType == double.class
